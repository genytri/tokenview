...
def test_parse_session_first_event_has_sessionid_no_timestamp(tmp_path: Path) -> None:
    """Real Claude Code sessions open with a {type,sessionId,aiTitle} event that
    carries no timestamp — the parser must still recover started_at from a later
    event instead of crashing on event["timestamp"]."""
    f = tmp_path / "session.jsonl"
    header = '{"type":"summary","sessionId":"sess-1","aiTitle":"Untitled"}'
    later = (
        '{"type":"assistant","timestamp":"2026-05-26T10:00:01.000Z",'
